    ...
    <div id="mockList">
        <div class="panel panel-default">
            <div class="panel-heading">配置
                <button type="button" class="btn btn-default btn-sm" aria-label="Left Align" data-toggle="modal" data-target="#editModal"
                    data-type="create">
            ...
            <div v-for="mockset in mocksets | filterBy filterMenu in 'menuId'" track-by="_id" v-bind:class="{'active':mockset.active,'opened':mockset.opened,'mockDiv':true,'POST':mockset.type=='POST','GET':mockset.type=='GET'}">
                <div class="mocksetHeader" v-on:click="togglePane">
                    <span class="mockType">{{mockset.type}}</span><a href="javascript:;" v-on:click.stop="testMock(mockset)"><code>{{mockset.url}}</code></a>
                    <span class="text-info">{{mockset.desc}}</span>
                    <div class="operator">
                        <button type="button" class="btn btn-default btn-xs" aria-label="Left Align" data-toggle="modal" data-target="#editModal"
                            data-id={{mockset._id}}>
...
let getURLParam = function(name, search) {
    return decodeURIComponent((new RegExp('[?|&]' + name + '=' + '([^&;]+?)(&|#|;|$)').exec(search || location.search) ||
        [true, ""])[1].replace(/\+/g, '%20')) || null;
};
